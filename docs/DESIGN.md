...
**Letra grande do sistema (sessão 28).** O Android deixa aumentar a letra até 2×; os ecrãs do Bricklap já são desenhados para se lerem de relance e a 2× nada cabe. Regra: **todo o texto acompanha a letra do sistema até 1,3×, e não mais** (`Texto` e `Escrita`, em `apps/mobile/ui/texto.tsx`; nenhum ecrã importa `Text` do react-native). E **nenhum desenho depende de uma palavra caber numa coluna estreita**: um rótulo tem a linha só para ele, ou tem uma linha só e encolhe (`TextoJusto` — botões, títulos, o cronómetro, que assim também aguenta `1:03:19`). Uma palavra partida a meio ("Repetiç/ões", "declarad/o", no telemóvel real na sessão 27) é um defeito de desenho, não de tradução.
...
- O ecrã de gravação tem **cinco alvos e nenhum mais**: Nova ronda e Registar (56), Marca (80), Mudar e Parar (56). Eram três até à sessão 26; as rondas e os valores (ADR 0011) trouxeram os outros dois.
- **Margens do sistema (sessão 28).** Edge-to-edge é obrigatório no Android 16: a app desenha por baixo da barra de estado e da de navegação. As margens **leem-se da janela** (`useMargens`, sobre `react-native-safe-area-context`) e nunca se supõem — até à sessão 27 o fundo era a constante 48 + 8, e as folhas nem essa usavam. Regra: nenhum alvo de toque fica, no todo ou em parte, por baixo de uma barra do sistema, **em nenhum ecrã e em nenhuma folha**; o último alvo de cada ecrã fica a 8 px do inset de baixo.
- **Teclado.** Em edge-to-edge o `adjustResize` já não encolhe a janela: quem se desvia do teclado é a app. Uma folha com campos de escrita assenta **em cima do teclado**, com o rodapé (a ação principal) sempre à vista.
...
- **Decisão do CTO, 2026-09-19 (sessão 28): a Marca passa a toque simples, com um "Anular" de 5 s (evento `mark_cancelled`) — [ADR 0011 §5](adr/0011-rondas-e-valores-registados.md).** A razão da sessão 14 só pesava um dos dois erros de um botão que se dispara ao premir: a marca falsa (parte um bloco em dois, vê-se logo, não perde nada) e a marca perdida (funde dois blocos, só se vê olhando para o anel, perde informação que não se recupera). **É trabalho de motor e não está implementado: até lá, a Marca é de premir e manter, como descrito acima.** O Parar mantém o gesto longo. Quando entrar, o anel sai da Marca e fica só no Parar.

**Folha.** O que sobe do fundo — a escolha de desporto e a ficha de valores (`apps/mobile/ui/folha.tsx`). Raio 24 em cima, pega de 36×4, véu por trás; fecha com toque no véu ou com o gesto de voltar. **Três zonas**: cabeça e rodapé fixos, o corpo desliza entre eles — o rodapé nunca desliza. **Não é um `Modal`** (sessão 28): um `Modal` é outra janela do Android, que em edge-to-edge não encolhe com o teclado nem recebe as margens do sistema, e foi por isso que o teclado tapou a ficha inteira e o Guardar ficou debaixo da barra de navegação. É uma vista absoluta dentro do ecrã: com o teclado fechado assenta no fundo e o rodapé leva o inset por dentro; com o teclado aberto assenta em cima dele e pode crescer até à barra de estado. O que está por baixo esconde-se dos leitores de ecrã.

**Escolha de desporto (folha).** O que o botão Mudar abre — sessão 17, reversão de uma decisão de âmbito do CTO na sessão 14 (o Mudar passava para o desporto seguinte de uma lista fixa, em ciclo, sem o fundador o ter pedido; cada toque escrevia um evento). Sobe do fundo, raio 24 em cima, pega de 36×4; os oito desportos em duas grelhas — INTERIOR e RUA (até à sessão 27 a primeira chamava-se GINÁSIO; ver abaixo), a mesma ordem e os mesmos ícones do início —, alvos de **56 px**. O desporto atual aparece com o selo "Atual", em `acento-fundo`/`acento`, e **não é premível**: não existe "mudar para o mesmo". **Nenhum evento se escreve ao abrir a folha** — só ao escolher um desporto diferente, e nessa ordem exata: a amostra de fronteira primeiro, o `changeSport` depois, como o CHANGE sempre fez. Cancelar — toque no véu por fora, o botão "Cancelar" no fundo da folha, ou o gesto de voltar do Android — fecha sem tocar na sessão. `apps/mobile/ui/escolhaDesporto.tsx`.
...
**Chip.** Pílula de **56 px** — só existe na ficha, que se toca durante o treino —, contorno `linha`; ativo = fundo `tinta`, texto `fundo`, uma linha só com reticências. Serve para escolher o exercício; a variante **a meias** (duas pílulas a repartir a linha) serve as escolhas entre dois: este bloco ou o anterior, peso livre ou peso do corpo, velocidade ou distância.

**Ficha rápida (folha).** Uma Folha, até 93 % da altura livre. Cabeça fixa com ícone do desporto + nome do bloco e uma linha de contexto ("Bloco 2 · 00:20 · Ronda 1"). No máximo **três campos**. **Cada campo tem o rótulo numa linha só dele e, por baixo, − [valor] + com 56 px cada** (sessão 28: com o rótulo à esquerda dos controlos sobravam-lhe 44 px e "Repetições" partia-se a meio); o valor vai a 29/800 numa caixa que ocupa o espaço entre o − e o +, com a unidade colada, e a caixa inteira abre o teclado. Um campo **calculado** mostra o valor sem controlo e diz de onde vem. Rodapé **fixo**: "Depois" (fantasma) e "Guardar" (acento) — com o teclado aberto ficam em cima dele, e **escreve-se um valor e guarda-se sem fechar o teclado**. O teclado numérico salta de campo em campo ("seguinte") e o último fecha-o; ao abrir, o corpo desliza até ao campo em escrita. Num bloco de exercícios as **sugestões são uma fila só, que desliza para o lado** e sangra até à margem da folha, e o tipo são duas pílulas a meias: num ecrã de 830 px de altura os campos de valor ficam à vista sem deslizar (em grelha, oito pílulas de 56 px eram 250 px).
...
**Gravação: o bloco aberto e os fechados (sessão 28).** O cartão por baixo do cronómetro **é o bloco aberto, a cabeça da lista de blocos**: rótulo "BLOCO 3 · A DECORRER" na cor do desporto, o nome do bloco — o exercício registado, ou o desporto — a 28/800 numa linha que encolhe, o tempo do bloco a 38/700 e, **mal se registam, os valores** a 22/800 com a marca `decl.`. Até à sessão 27 o cartão dizia o desporto três vezes (topo, rótulo e título) e registar valores no bloco aberto não mudava nada no ecrã até ele fechar. Por baixo, a lista chama-se pelo nome, **"BLOCOS FECHADOS"** com o contador, e mostra os três últimos; sem blocos fechados não se desenha nada — o cartão é o conteúdo. A pílula "Bloco n" do topo saiu (o cartão já o diz); a "Ronda n" fica. **A fiada conta o bloco aberto**: sem marcas era uma barra vazia, e o último troço a crescer é o treino a acontecer.

**Linha de bloco.** Duas linhas desde a sessão 28: `ícone + nome … tempo` em cima e, por baixo do nome, `valor` + `decl.`. Numa linha só, "500 m · 1:10/500 DECL." deixava o nome em "Re…" no telemóvel real — e o nome é o que diz que bloco é. Um bloco sem valores ocupa uma linha.

**Grupos de desportos: INTERIOR e RUA.** Decisão do fundador (sessão 28): com o desporto "Ginásio" (sessão 27), o cabeçalho do grupo dos desportos sem GPS deixou de se poder chamar GINÁSIO — muda o cabeçalho, fica o botão, porque o botão é o que se toca e o cabeçalho é arrumação. Entre "SEM GPS" e "INTERIOR" ficou **INTERIOR** ("Indoors"): faz par com RUA — dois sítios, que é como o atleta pensa quando escolhe —, lê-se de relance e não fala da máquina. "Sem GPS" é mais exato (uma piscina pode ser ao ar livre) mas descreve o mecanismo da app e não o mundo de quem treina; e a consequência de não haver GPS já tem o seu sítio, que é a marca `decl.` nos valores.
...
**Tipo de exercício (ficha).** Num bloco de exercícios, por baixo do nome e das sugestões do catálogo, duas pílulas a meias, "Peso livre" e "Peso do corpo", e por baixo uma linha a dizer o que a ativa pede ("Peso do corpo: só repetições") ([ADR 0012](adr/0012-taxonomia-de-exercicios.md)). A ativa segue o nome escrito quando o catálogo o conhece; o atleta muda-a com um toque, e é assim que um exercício novo ganha tipo. A ficha mostra **só os campos do tipo** — no máximo três, como já era regra.
