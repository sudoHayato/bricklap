...
- [ ] **Marca: em standby**, por decisão do fundador (não desta sessão). A exploração está em `tools/marca-blender/` e nos relatórios das sessões 18, 19 e 19b (fundida em `main` na sessão 21; a branch foi apagada na 22b): a **P2122** é a candidata das sessões 18/19 ([relatório](reports/2026-09-13-sessao-19.md)), e a **ligadura BL** — o B a partilhar a haste e o pé do L, a uma cor — é uma direção ainda por testar, não desenhada.
...
- [ ] **Remendo ao `expo-task-manager`** (`patches/expo-task-manager+57.0.17.patch`, sessão 08; **regenerado na sessão 22b** — tinha 668 artefactos de build do Gradle com caminhos de disco, e ficou só com a correção em `TaskService.java`, 36 linhas; **ao regenerá-lo, excluir `android/build/`**: `npx patch-package expo-task-manager --exclude 'package\.json$|android/build/'`): gestor de tarefas perdido quando a app abre no motor React headless. Tirar quando houver correção a montante (issue no ADR 0010) e rever a cada atualização do SDK. **À quarta surpresa na camada expo, módulo Kotlin** (regra do CTO).
