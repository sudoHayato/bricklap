...
- **Nova ronda** quando um circuito recomeça (sessão 26, ADR 0011): é mais um evento no registo, e o resumo agrupa os blocos por ronda. Carregar no mesmo instante de um Mudar não cria nenhum bloco de ~0 s. **Desde a sessão 27 a ronda 1 abre com o Iniciar**: a pílula diz "Ronda 1" desde o primeiro segundo e o botão diz a ronda que abre ("Nova ronda · 2"); um toque em cima do Iniciar não escreve nada.
- **Exercícios com tipo** (sessão 27, [ADR 0012](../../docs/adr/0012-taxonomia-de-exercicios.md)): na ficha de um bloco de Ginásio escreve-se o nome (ou escolhe-se uma sugestão do catálogo) e o tipo — peso livre mostra repetições e carga, peso do corpo só repetições. Um exercício novo entra assim, e fica nas sugestões das sessões seguintes.
- **"decl."** ao lado de um número (sessão 27, ADR 0011 §3a): foi declarado pelo atleta, ou calculado a partir de um valor declarado — o ritmo incluído. Um número medido pelo GPS nunca a leva, e os dois nunca se somam.
