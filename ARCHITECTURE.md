...
- **Exercício** (ADR 0012) — um dado, não código: `CatalogExercise {id, name, kind, aliases, seed}`. O catálogo de partida (`SEED_EXERCISES`) são os exercícios reais do fundador; o do atleta deriva-se do registo (`catalogFromMentions`). O **tipo** (`ExerciseKind`: `free_weight`, `bodyweight`) é código, porque decide os campos de um bloco de `strength` (`VALUE_FIELDS_BY_KIND`).
